...
({
    ...
    onInit : function(component, event, helper){
        ...
        var getSobjectValues = component.get('c.getsObjects');
        console.log('<<< get sobject ', getSobjectValues);
        console.log('1');
        getSobjectValues.setCallback(this, function(response){
            console.log('2');
                var sObjectList = JSON.parse(response.getReturnValue());
        ...
        // getSobjectValues.setStorable();
        $A.enqueueAction(getSobjectValues);
    },
    onSelectChange : function(component, event, helper){

        var selectedObj = component.find('selectObject').get('v.value');
        ...
        getFieldSet.setParams({
            "sObjectName" :  selectedObj
        });
        getFieldSet.setCallback(this, function(response){
            console.log("fieldsSetListObject ",response.getReturnValue());
            var state = response.getState();
            if(state === 'SUCCESS' || state === 'DRAFT'){
                var fieldsSetList = response.getReturnValue();
                console.log("fieldsSetListObject ",fieldsSetList);
                var listOptions = [];
                listOptions.push({
                    label : '--Select One--',
                    value : ''
